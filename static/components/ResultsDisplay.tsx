...
export const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ data, getAdviceColor }) => {
  ...
  const historicalDataEndIndex = mainChartData.findIndex(d => d.forecast !== undefined && d.forecast !== null && d.actual === undefined);
        ...
        <div className="w-full h-96">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={mainChartData} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              ...
              <Legend />
              <Line type="monotone" dataKey="actual" name="Actual Price" stroke="#3b82f6" strokeWidth={2} dot={false} connectNulls={false} />
              <Line type="monotone" dataKey="forecast" name="Forecasted Price" stroke="#f59e0b" strokeWidth={2} dot={false} connectNulls={false} />
              {historicalDataEndIndex > 0 && (
                 <ReferenceArea x1={mainChartData[0].ds} x2={mainChartData[historicalDataEndIndex -1 < 0 ? 0 : historicalDataEndIndex -1].ds} strokeOpacity={0.3} label={{value: "Historical Data", position:"insideTopLeft", fill:"#6b7280", fontSize:10, dy:-10 }} />
              )}
              {historicalDataEndIndex >= 0 && historicalDataEndIndex < mainChartData.length && ( // Changed >0 to >=0 to cover case where all data is forecast
              ...
              <Line type="monotone" dataKey="lowerBound" name="Lower Bound" stroke="#a1a1aa" strokeWidth={1} strokeDasharray="5 5" dot={false} connectNulls={false} />
              <Line type="monotone" dataKey="upperBound" name="Upper Bound" stroke="#a1a1aa" strokeWidth={1} strokeDasharray="5 5" dot={false} connectNulls={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </section>
